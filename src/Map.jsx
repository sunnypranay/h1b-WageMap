...
export default function Map() {
  ...
  function showCountyPopup(feature, lngLat) {
    ...
    const chance =
      currentLevel > 0 ? lottery.selectionByLevel[currentLevel] : undefined;
    ...
    const salaryFloors = levels.keys.map((k) => {
      ...
      return `<div>${formatAnnual(annual)}+</div><div class="hourly-col">${formatHourly(
        hourly
      )}</div>`;
    });

    const levelRows = levels.keys
      .map((k, idx) => {
        const levelNumber = idx + 1;
        const salaryRange = salaryFloors[idx];
        const rowChance = lotteryOn
          ? lottery.selectionByLevel[levelNumber]
          : null;
        const chanceCell =
          lotteryOn && rowChance !== null
            ? `<td class="chance-col">${
                Number.isFinite(rowChance)
                  ? `${rowChance}%`
        ...
        return `<tr class="${
          currentLevel === levelNumber ? "is-active-row" : ""
                ...
                <td class="level-col">${levels.labels.tableLevelPrefix} ${k}</td>
    ...
    const popup = new maplibregl.Popup({
      ...
      .setHTML(
        `<div class="county-popup-content">
           <div class="popup-top">
             <div>
               <div class="popup-title">${featureLabel(feature)}, ${STATE_ABBR_TO_NAME[state] || state}</div>
             </div>
           ...
           <div class="level-table-wrapper">
             <table class="level-table" role="table">
               <thead>
                 <tr>
                   <th>${levels.labels.tableHeaders.level}</th>
                   <th>${levels.labels.tableHeaders.salary}</th>
                   ${
                     lotteryOn
                       ? `<th>${levels.labels.tableHeaders.probability}</th>`
                       : ""
                   }
                 </tr>
               </thead>
  ...
  function zoomToState(stateAbbr) {
    ...
    countiesRef.current.features
      .filter((f) => featureState(f) === stateAbbr)
      .forEach((f) => {
  ...
  useEffect(() => {
    const { soc: initialSoc, socParam, socText: initialText } =
    ...
    fetch(data.socCodesUrl)
      ...
      .then((options) => {
        ...
        setSoc((current) =>
          current === initialSoc || current === resolved.parent
            ? resolved.parent
            : current
        );
        setSocText((current) =>
          current === initialText || current === socParam
            ? resolved.display
            : current
        );
      })
  ...
  useEffect(() => {
    ...
    if (!targetState && targetCounty) {
      ...
      if (feature) {
        targetState = featureState(feature);
      }
    }
    ...
    if (targetCounty) {
      handleCountyChange(targetCounty);
    }
  ...
  useEffect(() => {
    ...
    if (salary !== "" && Number.isFinite(Number(salary))) {
      params.set(query.keys.salary, String(Number(salary)));
    } else {
      params.delete(query.keys.salary);
    }
  ...
  const salaryDisplay =
    salary === "" || Number.isNaN(Number(salary))
      ? formatting.emptyValue
      : `${formatting.currencySymbol}${formatCurrency(
          Number(salary)
        )}${formatting.annualSuffix}`;
