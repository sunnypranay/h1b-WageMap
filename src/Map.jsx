...
import { buildLocationIndex } from "./locationSearch";
...
function parseCompareParam(raw) {
  if (!raw) return [];
  return String(raw).split(",").map((item) => item.trim()).filter(Boolean).slice(0, 2);
}

function parseQueryState() {
  if (typeof window === "undefined") {
    return {
      hasQuery: false, state: "", county: "", soc: defaults.soc, socParam: defaults.soc,
      socText: defaults.socText, salary: defaults.salary, lotteryEnabled: false, compare: [],
    };
  }
  ...
  const rawCmp = params.get(query.keys.cmp);
  ...
  const socText =
    !rawSoc || wageSocFromParam(socParam) === defaults.soc ? defaults.socText : socParam;
  const salary =
    rawSalary && Number.isFinite(Number(rawSalary)) ? Number(rawSalary) : defaults.salary;
  const lotteryEnabled = query.truthyValues.includes(String(rawH1b).toLowerCase());
  return {
    hasQuery, state, county, soc, socParam, socText, salary, lotteryEnabled,
    compare: parseCompareParam(rawCmp),
  };
}
...
export default function Map() {
  ...
  const [selectedState, setSelectedState] = useState(initialQueryRef.current.state);
  const [selectedCounty, setSelectedCounty] = useState(initialQueryRef.current.county);
  const [lotteryEnabled, setLotteryEnabled] = useState(initialQueryRef.current.lotteryEnabled);
  const [wagesLoading, setWagesLoading] = useState(true);
  const [locationIndex, setLocationIndex] = useState([]);
  const [insights, setInsights] = useState({ highLevel: [], cheapestLevelII: [] });
  const [compareIds, setCompareIds] = useState(initialQueryRef.current.compare);
  const wageRequestRef = useRef(0);

  function handleShare() {
    const url = window.location.href;
    if (navigator.share) navigator.share({ title: share.title, text: share.text, url });
    else {
  ...
  useEffect(() => {
    ...
    mapRef.current = map;
    map.on("load", async () => {
      const [countyRes, geoidRes, citiesRes] = await Promise.all([
        fetch(data.countiesUrl),
        fetch(data.geoidsUrl),
        fetch(data.citiesUrl),
      ]);
      ...
      prepareLocationData(counties, cities);
      const hatchSize = 16;
  ...
  function prepareLocationData(countiesGeojson, cities) {
    ...
    countiesGeojson.features.forEach((f) => {
      ...
      byState[abbr].push({ name: featureLabel(f), geoid: f.properties.GEOID });
      countyEntries.push({ name: featureLabel(f), state: abbr, geoid: f.properties.GEOID });
    });
    Object.values(byState).forEach((list) => list.sort((a, b) => a.name.localeCompare(b.name)));
    countiesByStateRef.current = byState;
    countyFeatureMapRef.current = featureMap;
    setLocationIndex(buildLocationIndex(cities, countyEntries));
  ...
  function clearActivePopup() {
    ...
    refreshPickedOutlines();
  }

  function fitPadding() {
    if (typeof window === "undefined") return mapConfig.fitBounds.padding;
    const mobile = window.matchMedia("(max-width: 640px)").matches;
    if (mobile) return { top: 12, right: 12, bottom: 12, left: 12 };
    return { top: 24, right: 28, bottom: 24, left: mapConfig.fitBounds.panelLeft };
  }

  function refreshPickedOutlines() {
    const map = mapRef.current;
    if (!map || !map.getSource(mapConfig.sources.counties)) return;
    const next = new Set([activeFeatureRef.current?.geoid, ...compareIdsRef.current].filter(Boolean));
    pickedIdsRef.current.forEach((geoid) => {
      if (!next.has(geoid)) {
        map.setFeatureState({ source: mapConfig.sources.counties, id: geoid }, { picked: false });
      }
    });
    next.forEach((geoid) => {
      map.setFeatureState({ source: mapConfig.sources.counties, id: geoid }, { picked: true });
    });
    pickedIdsRef.current = Array.from(next);
  }

  function refreshInsights(annualSalary) {
    const annual = Number(annualSalary);
    if (!countiesRef.current || !wageTableRef.current || !Number.isFinite(annual)) {
      setInsights({ highLevel: [], cheapestLevelII: [] });
      return;
    }
    setInsights(buildCountyLists(
      countiesRef.current.features, wageTableRef.current, annual, wage.hoursPerYear, featureLabel, featureState
    ));
  }
  ...
  async function updateLevels(selectedSoc, annualSalary) {
    ...
    try {
      ...
      mapRef.current.setPaintProperty(mapConfig.layers.countyFill, "fill-color", [
        "case",
        ["==", ["get", "level"], 4], levels.colors[4],
        ["==", ["get", "level"], 3], levels.colors[3],
        ["==", ["get", "level"], 2], levels.colors[2],
        ["==", ["get", "level"], 1], levels.colors[1],
        ["==", ["get", "level"], 0], levels.colors[0],
        mapConfig.paint.noDataFill,
      ]);
      if (mapRef.current.getLayer(mapConfig.layers.countyNoData)) {
        mapRef.current.setFilter(mapConfig.layers.countyNoData, ["!", ["has", "level"]]);
      }
      refreshInsights(annual);
      refreshPickedOutlines();
  ...
  function gapCopy(levelInfo, annualSalary) {
    const gap = nextLevelGap(levelInfo, Number(annualSalary));
    if (!gap) return "";
    if (gap.atMax) return ui.popup.atMax;
    const amount = formatAnnual(gap.gap);
    if (gap.levelNumber === 1) return ui.popup.belowAll.replace("{{amount}}", amount);
    return ui.popup.gapToNext.replace("{{amount}}", amount).replace("{{level}}", gap.key);
  }

  function areaCopy(geoid) {
    const peers = sameWageAreaPeers(
      geoid, wageTableRef.current, countyFeatureMapRef.current, featureLabel, featureState
    );
    if (!peers.length) return ui.popup.sameAreaNone;
    const shown = peers.slice(0, 4).map((peer) => `${peer.name}, ${peer.state}`);
    const remaining = peers.length - shown.length;
    if (remaining <= 0) return ui.popup.sameAreaOne.replace("{{names}}", shown.join("; "));
    return ui.popup.sameAreaMany.replace("{{names}}", shown.join("; ")).replace("{{count}}", String(remaining));
  }
  ...
  function showCountyPopup(feature, lngLat) {
    ...
    const levelInfo = wageTable ? wageTable[geoid] : null;
    ...
    const levelClass =
      !hasLevelData || currentLevel === 0 || currentLevel === undefined ? "level-none" : "has-level";
    const levelColor =
    ...
    const gapLine = hasLevelData ? gapCopy(levelInfo, salary) : "";
    const areaLine = hasLevelData ? areaCopy(geoid) : "";
    const comparing = compareIdsRef.current.includes(geoid);
    const selectionLine =
      lotteryOn && chance
    ...
    const popup = new maplibregl.Popup({
      ...
      .setLngLat(point)
      .setHTML(`<div class="county-popup-content"><div class="popup-top"><div><div class="popup-title">${escapeHtml(featureLabel(feature))}, ${escapeHtml(STATE_ABBR_TO_NAME[state] || state)}</div></div><div class="popup-top-actions"><span class="level-badge ${levelClass}"><span class="level-dot" style="background:${levelColor};"></span><span class="level-badge-text">${escapeHtml(levelLabel)}</span></span><button type="button" class="popup-close" aria-label="Close">×</button></div></div>${selectionLine ? `<div class="selection-line">${selectionLine}</div>` : ""}${gapLine ? `<div class="popup-insight">${escapeHtml(gapLine)}</div>` : ""}${areaLine ? `<div class="popup-area">${escapeHtml(areaLine)}</div>` : ""}<div class="level-table-wrapper"><table class="level-table" role="table"><thead><tr><th>${levels.labels.tableHeaders.level}</th><th>${levels.labels.tableHeaders.salary}</th>${lotteryOn ? `<th>${levels.labels.tableHeaders.probability}</th>` : ""}</tr></thead><tbody>${levelRows}</tbody></table></div><div class="popup-actions"><button type="button" class="popup-compare">${comparing ? escapeHtml(ui.compare.addedLabel) : escapeHtml(ui.compare.addLabel)}</button></div></div>`)
    ...
    popup.getElement()?.querySelector(".popup-compare")?.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      toggleCompare(geoid);
    });
    activeFeatureRef.current = { geoid, point };
  ...
  function applyCountySelection(geoid, options = {}) {
    const feature = countyFeatureMapRef.current[geoid];
    if (!feature) return;
    const state = featureState(feature);
    setSelectedState(state);
    setSelectedCounty(geoid);
    setCountyOptions(countiesByStateRef.current[state] ?? []);
    if (!options.skipZoom) zoomToCounty(geoid);
    showCountyPopup(feature, options.lngLat);
  }

  function selectCountyFromFeature(feature, lngLat) {
    if (!feature?.properties?.GEOID) return;
    applyCountySelection(feature.properties.GEOID, { lngLat, skipZoom: true });
  }
  ...
  function handleCountyChange(nextCounty) {
    if (nextCounty) applyCountySelection(nextCounty);
    else {
      setSelectedCounty("");
      clearActivePopup();
    }
  ...
  function handleLocationPick(item) {
    if (!item?.geoid) return;
    applyCountySelection(item.geoid);
  }

  function handleRegionJump(regionId) {
    const bounds = mapConfig.regions?.[regionId];
    if (!bounds) return;
    if (regionId === "CONUS") {
      setSelectedState("");
      setSelectedCounty("");
      ...
      handleStateChange(regionId);
    }
    fitToBounds(bounds);
  }

  function handleOpenCounty(geoid) {
    applyCountySelection(geoid);
  }

  function toggleCompare(geoid) {
    setCompareIds((current) => {
      if (current.includes(geoid)) return current.filter((id) => id !== geoid);
      const next = [...current, geoid];
  ...
  useEffect(() => {
    ...
    if (compareIds.length) params.set(query.keys.cmp, compareIds.join(","));
    else params.delete(query.keys.cmp);
    const nextQuery = params.toString();
    const nextUrl = `${window.location.pathname}${nextQuery ? `?${nextQuery}` : ""}${window.location.hash}`;
    window.history.replaceState(null, "", nextUrl);
  }, [selectedState, selectedCounty, soc, salary, lotteryEnabled, compareIds]);
  ...
  const compareCounties = compareIds.map((geoid) => {
    const feature = countyFeatureMapRef.current[geoid];
    const floors = wageTableRef.current?.[geoid] || null;
    return {
      geoid,
      name: feature ? featureLabel(feature) : geoid,
      state: feature ? featureState(feature) : "",
      floors,
  ...
  return (
    <>
      <ControlPanel
        ...
        insights={insights}
        compareCounties={compareCounties}
        onOpenCounty={handleOpenCounty}
        onRemoveCompare={(geoid) => setCompareIds((current) => current.filter((id) => id !== geoid))}
      />
      {wagesLoading && (
        <div className="map-loading-chip" role="status" aria-live="polite">
          {ui.mapStatus.loading}
        </div>
      )}
      <div id="map" />
    </>
