...
import { formatAnnual, formatCurrency } from "./mapUtils";
...
export default function ControlPanel({
  ...
  return (
    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          <div className="section panel-card">
            ...
            <div className="row">
              <SocAutocomplete value={socText} onSelect={onSocSelect} autoFocus />
              <OccupationHelp />
            </div>
          </div>
          ...
          <div className="section panel-card">
            ...
            <div className="salary-row">
              <div className="salary-input">
                ...
                />
                {salary !== "" && (
                  <button type="button" className="clear-btn" aria-label={ui.salary.clearLabel} onClick={onClearSalary}>
                    ×
                  </button>
                )}
              </div>
          ...
          <div className="section panel-card">
            <div className="toggle-row">
              <div className="toggle-copy">
                <div className="toggle-subtitle">{lottery.toggleQuestion}</div>
                <div className="toggle-disclaimer">{lottery.disclaimer}</div>
              </div>
              <div className="toggle-stack">
                <button type="button" className={`toggle ${lotteryEnabled ? "on" : ""}`} aria-pressed={lotteryEnabled} onClick={onToggleLottery} aria-label={lottery.toggleAriaLabel}>
                  <span className="toggle-handle" />
                </button>
                <span className="toggle-state-text">{lotteryEnabled ? lottery.toggleOnLabel : lottery.toggleOffLabel}</span>
              </div>
            </div>
          </div>
        </>
      ...
      {collapsed && (
        <div className="pinned-summary" title={ui.pinned.title} onClick={onToggleCollapse} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") onToggleCollapse(); }}>
          <div className="pinned-row"><span>{ui.pinned.occupation}</span><span className="pinned-value">{occupationDisplay}</span></div>
          <div className="pinned-row"><span>{ui.pinned.salary}</span><span className="pinned-value">{salaryDisplay}</span></div>
        </div>
      )}

      {!collapsed && (
        <>
          <div className="section panel-card insights-card">
            <div className="label-row"><label className="label">{ui.insights.title}</label></div>
            {!(insights?.highLevel?.length || insights?.cheapestLevelII?.length) ? (
              <div className="insight-empty">{ui.insights.empty}</div>
            ) : (
              <div className="insight-grid">
                <div>
                  <div className="insight-heading">{ui.insights.highLevelTitle}</div>
                ...
                <div>
                  <div className="insight-heading">{ui.insights.cheapestTitle}</div>
                  <ul className="insight-list">
          ...
          <div className="section panel-card">
            <div className="label-row">
              <label className="label">{ui.compare.title}</label>
              <span className="hint">{ui.compare.hint}</span>
            </div>
            {(!compareCounties || compareCounties.length === 0) && (
              <div className="insight-empty">{ui.compare.empty}</div>
            )}
                  ...
                  <tbody>
                    {levels.keys.map((key) => (
                      <tr key={key}>
                        <td>L{key}</td>
                        {compareCounties.map((county) => (
                          <td key={`${county.geoid}-${key}`}>
                            {Number.isFinite(county.floors?.[key])
                              ? formatAnnual(county.floors[key] * SITE_CONFIG.wage.hoursPerYear)
        ...
        <>
          <div className="footer">
            <div className="footer-left">
              <button type="button" onClick={handleShare} aria-label={ui.share.ariaLabel} title={ui.share.title} className="icon-button">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              ...
              <a className="icon-link" href={links.repoUrl} target="_blank" rel="noopener noreferrer" aria-label={links.repoLabel} title={links.repoLabel}>
            ...
            <a className="disclosures-link" href={links.disclosures} target="_blank" rel="noopener noreferrer">{links.disclosuresLabel}</a>
            <a className="oflc-link" href={links.oflc} target="_blank" rel="noopener noreferrer">{links.oflcLabel}</a>
          </div>
          <div className="credit">
            <a href={links.authorUrl} target="_blank" rel="noopener noreferrer">{links.authorName}</a>
          </div>
        </>
      )}
    </div>
